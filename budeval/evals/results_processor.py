...
class ResultsProcessor:
    ...
    def extract_from_pvc(self, job_id: str, namespace: str = "budeval", kubeconfig: Optional[str] = None) -> str:
        ...
        uuid_part = job_id.split('-', 1)[-1] if '-' in job_id and job_id.startswith(('opencompass-', 'vllm-', 'lighteval-')) else job_id
        ...
        try:
            ...
            if result.returncode != 0:
                raise Exception(f"PVC {pvc_name} not found in namespace {namespace}. Cannot extract results from missing PVC.")
                
            logger.info(f"PVC {pvc_name} found, proceeding with extraction")
        except subprocess.TimeoutExpired:
            raise Exception(f"Timeout checking for PVC {pvc_name}")
        except Exception as e:
            logger.error(f"Failed to verify PVC {pvc_name} exists: {e}")
            raise Exception(f"Cannot extract results: PVC verification failed - {e}")
