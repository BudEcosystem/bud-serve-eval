...
def check_kubernetes_engines():
    ...
    try:
        ...
        result = subprocess.run(
            ["kubectl", "get", "configmap", "preloaded-engines", "-n", "budeval", "-o", "json"],
            capture_output=True,
            text=True,
        )
...
def check_running_jobs():
    ...
    try:
        ...
        result = subprocess.run(
            ["kubectl", "get", "jobs", "-n", "budeval", "-l", "purpose=image-preloader", "--no-headers"],
            capture_output=True,
            text=True,
        )
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            for job in jobs:
                ...
                logs_result = subprocess.run(
                    ["kubectl", "logs", "-n", "budeval", f"job/{job_name}", "--tail=3"], capture_output=True, text=True
                )
